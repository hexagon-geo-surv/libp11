...
int PKCS11_set_password_callback(PKCS11_CTX *pctx, OSSL_PASSPHRASE_CALLBACK *pw_cb, void *pw_cbarg)
{
    PKCS11_CTX_private* ctx = PRIVCTX(pctx);
    if (check_fork(ctx) < 0)
        return -1;

    return pkcs11_set_password_callback(ctx, pw_cb, pw_cbarg);
}
                                ...
                                unsigned long **mechcountsp)
{
    ...
    if (mechcountsp)
        *mechcountsp = 0;
    return pkcs11_enumerate_mechanisms(ctx, slots, nslots, mechsp, mechcountsp);
}
